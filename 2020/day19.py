...
n=0
while n<len(rec[0]):
    g = (i for i, e in enumerate(rec[0][n]) if isinstance(e, int))
    num=test(g)
    if isinstance(num, int):
        ...
        #del p[num]
        for el in r:
            rec[0].append(p[:num]+el+p[num+1:])
            # p2=p.copy()
            # p2[num:0]=el
            # rec[0].append(p2)
            # for j in range(len(el)):
            #     p2.insert(num+j,el[j])
            # rec[0].append(p2)
        n=0
        #print(rec[0])
    else: n+=1
...
rules0=[''.join(r) for r in rec[0]]

print("Part 1:",sum(p in rules0 for p in phrase))
